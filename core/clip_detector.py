...
class ClipDetector:
    ...
    HOOK_KEYWORDS = [
        "shocking", "truth", "secret", "actually", "real", 
        "listen", "wait", "believe", "never", "always",
        "warning", "happened", "mistake", "wrong", "right",
        "surprising", "unexpected", "reveal", "breaking",
        "important", "critical", "essential", "key",
        "imagine", "think", "know", "guess", "guess what",
        "here's the thing", "the truth is", "let me tell you"
    ]
    
    # Controversial/emotional keywords
    EMOTION_KEYWORDS = [
        "angry", "furious", "love", "hate", "fear", "scared",
        "amazing", "incredible", "unbelievable", "insane",
        "crazy", "ridiculous", "absurd", "stupid", "idiotic",
        "brilliant", "genius", "smart", "dumb", "ignorant",
        "finally", "finally!", "answer", "explain", "why"
    ]
    ...
    def detect_clips(
        ...
        # Step 4: Rank and select top clips
        final_clips = self._rank_and_select(candidate_clips)
        
        return final_clips[:self.num_clips]
    
    def _rule_based_scoring(self, transcript: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Apply rule-based scoring to each segment"""
        ...
        for i, seg in enumerate(transcript):
            ...
            # Short punchy sentences (engaging)
            word_count = len(text.split())
            if 3 <= word_count <= 15:
                score += 0.1
                reasons.append("punchy_length")
            
            # Question detection (engaging)
            if "?" in seg["text"]:
                score += 0.15
    ...
    def _merge_segments(
        ...
        for item in scored_segments:
            ...
            # Start new clip if score is high enough
            if score > 0.3:
    ...
    def _extract_topics(self, segments: List[Dict[str, Any]]) -> List[str]:
        ...
        topics = set()
        topic_keywords = {
            "politics": ["politics", "government", "president", "election", "policy", "vote"],
...
[
  {{
    ...
    def _merge_llm_and_rule(
        self,
        llm_clips: List[ClipSegment],
        rule_clips: List[ClipSegment]
    ) -> List[ClipSegment]:
    ...
    def _time_overlap(self, clip1: ClipSegment, clip2: ClipSegment) -> bool:
        ...
        return not (clip1.end_time <= clip2.start_time or clip1.start_time >= clip2.end_time)
